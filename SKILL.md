...
4. Replace every placeholder in `src/research.js`: date, research title, concise synthesis, optional repeated-workflow labels, a lead post when one deserves emphasis, the selected post IDs, author labels, section labels, the selection note, and both `reviewedPosts` lists. Classify every deduplicated public Post candidate considered during the research pass into exactly one list:
   - `notNoise`: selected Posts plus relevant Posts that were reviewed but not embedded. Give each a brief `note` saying what evidence or question it contributed.
   - `noise`: irrelevant, duplicative, promotional, unsupported, or otherwise excluded Posts. Give each a brief `note` stating the exclusion reason.
   Do not put the entire private bookmark corpus in these lists. Include a bookmark-derived Post only when it entered the merged candidate set, and never record its private-source provenance. The lists count unique candidates, while the manifest's public-read count remains the billed number of Post reads and may be larger because searches overlap. Copy the completed file to the archive as `research.js` and write `manifest.md` before rendering.
5. Use the bundled `react-tweet` component. Do not replace embeds with screenshots, hand-written tweet cards, or quoted text. The two reviewed-post lists start collapsed; their full embeds lazy-load only after the reader first opens each list.
